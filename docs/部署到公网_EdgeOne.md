...
| 平台 | 能不能跑本项目 |
|---|---|
| **腾讯云 EdgeOne Pages**（本方案） | ✅ 支持 Python 3.10 + Flask（WSGI 框架模式），免费额度每月 50GB 流量，连 GitHub 自动部署 |
...
- [ ] 一个 GitHub 账号（已有：`OFten99`）
- [ ] 一个腾讯云账号，且**已完成实名认证**（EdgeOne Pages 要求）
- [ ] 一个邮箱（注册 Aiven 用）
...
1. 打开 <https://console.aiven.io/signup>，用邮箱注册（免费、**不需要信用卡**）。
...
6. **这一步最容易漏，漏了第 2 步一定失败**：
   ...
   > `ERROR 1419`。本项目有 4 个触发器 + 18 个存储过程，必须先打开这个开关。
...
## 八、第 5 步：在 EdgeOne Pages 部署

1. 打开 <https://pages.edgeone.ai/> 并登录（腾讯云账号）。
2. **创建项目** → 选择 **导入 Git 仓库**。
3. 首次使用需授权 GitHub，授权后选择仓库 **`OFten99/hospital-system`**，
   分支选 **`main`**。
4. 构建配置：
   - **框架预设**：选「其他 / Other」（本项目没有前端构建步骤）
   - **构建命令**：留空
   - **输出目录**：留空或填 `/`
   - 本项目是「云函数」项目，静态部分为空，全部请求由 `cloud-functions/[[default]].py` 处理
5. **加速区域**：
   - 选「中国大陆可用区」国内访问最快，但绑定自定义域名需要 ICP 备案；
   - 用平台自动分配的 `xxx.edgeone.site` 域名不需要备案，够用了。
6. 点击部署，等 1~3 分钟，拿到访问地址形如
   `https://xxxx.edgeone.site`。
...
保存后 **重新部署一次**（环境变量改完需要重新部署才生效）。

> 在 **项目设置 → 函数管理** 里可以选云函数的运行地域，默认 `ap-guangzhou`（广州），
> 对国内访问最友好，保持默认即可。
...
访问 `https://xxxx.edgeone.site`，用下列账号登录（密码统一 `123456`）：
...
| 项目 | EdgeOne Pages 免费额度 |
|---|---|
| 流量 | 每月 50GB |
| 构建次数 | 不限 |
| 函数请求 | 每月 300 万次（边缘函数额度） |
| 单次执行时长 | 默认 30 秒，最大可配到 120 秒 |
| 代码包大小 | 128MB（本项目约 190KB，远远够用） |
...
1. **数据库口令只放在平台的环境变量里**，绝对不要写进代码或提交到 GitHub。
   本仓库的 `.gitignore` 已排除 `my_his.cnf`（里面有本地数据库口令）。
